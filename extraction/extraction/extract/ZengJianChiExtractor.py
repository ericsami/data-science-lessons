    ...
    def extract_change(self, paragraph):
        ...
        for target in targets:
            ...
            if m_com != None and len(m_com) > 0:
                shareholder = m_com[-1]
                if(shareholder == "上海证券交易所")  and len(m_com) > 1:
                    shareholder = m_com[-2]
            else :
...
class TableDictFieldPattern(object):
    ...
    def getDateFromText(text):
        ...
        if "月" in text or "年" in text or "/" in text or "." in text:
            if len(strList) < 2:
                strList = re.split("-|—|~", text)
            else:
                if "年" not in strList[-1] and "年" in strList[-2]:
                    strList[-1] = strList[-2].split("年")[0] + "年" + strList[-1]
        return strList[-1]
